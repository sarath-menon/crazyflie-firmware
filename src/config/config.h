...
#endif
#define FREERTOS_MIN_STACK_SIZE                                                \
  150 // M4-FPU register setup is bigger so stack needs to be bigger
 ...
 * This is the threshold for a battery and connector to pass. It loads the power
 * path by spinning all 4 motors and measure the voltage sag. The threshold is
 * very experimental and dependent on stock configuration. It is fairly constant
 * over the battery voltage range but testing with fully changed battery is
 * best.
 */
...
// Define to force initialization of expansion board drivers. For test-rig and
// programming.
...
#error                                                                         \
    "Can't define UART_OUTPUT_TRACE_DATA and ADC_OUTPUT_RAW_DATA at the same time"
#endif
...
#error                                                                         \
    "UART_OUTPUT_TRACE_DATA and T_LAUNCH_ACC doesn't work at the same time yet due to dma sharing..."
#endif
