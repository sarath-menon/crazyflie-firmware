...
static const uint8_t typeLength[] = {
    [LOG_UINT8] = 1, [LOG_UINT16] = 2, [LOG_UINT32] = 4, [LOG_INT8] = 1,
    [LOG_INT16] = 2, [LOG_INT32] = 4,  [LOG_FLOAT] = 4,  [LOG_FP16] = 2,
};
...
static int logCreateBlock(unsigned char id, struct ops_setting *settings,
                          int len);
static int logCreateBlockV2(unsigned char id, struct ops_setting_v2 *settings,
                            int len);
...
void logInit(void) {
  ...
  for (int i = 0; i < logsLen; i++) {
    ...
    if (logs[i].type & LOG_GROUP) {
      ...
      // CMD_GET_ITEM_V2 result's size is: 3 + strlen(logs[i].name) +
      // groupLength + 2
...
void logTOCProcess(int command) {
  ...
  case CMD_GET_ITEM: // Get log variable
    ...
    if (ptr < logsLen) {
      ...
      ASSERT(p.size <= CRTP_MAX_DATA_SIZE); // Too long! The name of the group
                                            // or the parameter may be too long.
  ...
  case CMD_GET_ITEM_V2: // Get log variable
    ...
    if (ptr < logsLen) {
      ...
      memcpy(p.data + 4 + strlen(group) + 1, logs[ptr].name,
             strlen(logs[ptr].name) + 1);
      crtpSendPacketBlock(&p);
    } else {
      LOG_DEBUG("    Index out of range!");
...
void logControlProcess() {
  ...
  case CONTROL_CREATE_BLOCK_V2:
    ret = logCreateBlockV2(p.data[1], (struct ops_setting_v2 *)&p.data[2],
                           (p.size - 2) / sizeof(struct ops_setting_v2));
    break;
  case CONTROL_APPEND_BLOCK_V2:
    ret = logAppendBlockV2(p.data[1], (struct ops_setting_v2 *)&p.data[2],
                           (p.size - 2) / sizeof(struct ops_setting_v2));
    break;
  }
...
static int logCreateBlock(unsigned char id, struct ops_setting *settings,
  ...
  logBlocks[i].id = id;
  logBlocks[i].timer =
      xTimerCreateStatic("logTimer", M2T(1000), pdTRUE, &logBlocks[i],
                         logBlockTimed, &logBlocks[i].timerBuffer);
...
static int logAppendBlock(int id, struct ops_setting *settings, int len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if ((currentLength + typeLength[settings[i].logType & LOG_TYPE_MASK]) >
        LOG_MAX_LEN) {
    ...
    if (settings[i].id != 255) // TOC variable
    {
      ...
      if (varId < 0) {
        LOG_ERROR("Trying to add variable Id %d that does not exists.",
                  settings[i].id);
        return ENOENT;
      }
...
static int logStartBlock(int id, unsigned int period) {
  ...
  // for high-rate logging (every millisecond)
  period = 1;
...
void logRunBlock(void *arg) {
  ...
  while (ops) {
    ...
    // We first copy the data to an (aligned) local variable, before assigning
    // it
...
static void opsFree(struct log_ops *ops) { ops->variable = NULL; }
...
void *logGetAddress(logVarId_t varid) { return logs[varid].address; }

uint8_t logVarSize(int type) { return typeLength[type]; }
